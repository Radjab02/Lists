#ifndef TIMER_H
#define TIMER_H
/* This file implements a high performance timer for measuring the performance
 ...
 *  TOCK()                  - Stop the timer
 *  TICK_ELAPSED()          - Return number of clock ticks elapsed between TICK() and TOCK()
 *  DURATION()              - Return duration between TICK() and TOCK() in seconds (double precision).
 *  SHOW_DURATION()         - Macro to show the duration using cout.
 *  MEASURE(THIS)           - Measure codes in THIS and show the duration
 *  MSHOW_DURATION(MESSAGE) - Same like SHOW_DURATION(), but show the duration with MESSAGE
 *  MMEASURE(MESSAGE, THIS) - Same like MEASURE(THIS), but show the duration with MESSAGE
 *
 * Created: 16 February 2006 9am, version 0.1
 * Author: Nazri Ramliy
 * Reference: http://www.geisswerks.com/ryan/FAQS/timing.html
 */
 ...
 /*  Sample usage:
   ...
   int main() {
     ...
     cout << "Duration 1 _sleep(2500) = " << duration1 << "s" << endl;
